...
bool check_packet_crc32(uint8_t *buffer, size_t recv_len, const char *sender_ip_address,
    ...
    if (!validate_crc32(buffer, recv_len - CRC32_LEN, received_crc)) {
        if (packet_type == TRANSMISSION_START_PACKET_TYPE) {
            send_acknowledgment(clientfd, sender_ip_address, sender_port, packet_type, false, DEFAULT_PACKET_INDEX);
            ...
            send_acknowledgment(clientfd, sender_ip_address, sender_port, packet_type, false, packet_index);
        } else {
            send_acknowledgment(clientfd, sender_ip_address, sender_port, packet_type, false, DEFAULT_PACKET_INDEX);
        }
        return false;
